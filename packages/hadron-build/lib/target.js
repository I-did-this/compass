...
class Target {
  ...
  configureForWin32() {
    ...
    this.installerOptions = {
      ...
      //
      // LOCAL PATCH (ephemeral, node_modules only): only request installer
      // signing when signing credentials are actually present. Without this,
      // electron-winstaller runs its bundled signtool.exe with no certificate
      // and the whole installer build fails. Omitting signWithParams makes it
      // produce an unsigned installer instead.
      signWithParams:
